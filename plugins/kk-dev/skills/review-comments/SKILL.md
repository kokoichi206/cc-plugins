...
5. **コメントへの返信**
   ...
   返信のトーン:
   - 「ご指摘ありがとうございます」「レビューありがとうございます」等の定型の挨拶・お礼は書かない
   - 構成は「メンション → SHA + 何をしたか → 一言ポイント」の 3 行構成とする
   ...
   - 「対応しました」の動詞部分は実際の変更に合わせて言い換える (例: 「分割しました」「句点を追加しました」)
   ...
   - 理由は結論ファーストで 1〜2 行に収める。経緯の説明・代替案の列挙・クッション言葉で膨らませない
   - 例:

   ```
   @coderabbitai
   ここは現状のままとします。
   このリトライは外部 API のレート制限が前提で、指数バックオフに変えると上限超過時の復帰がかえって遅くなるためです
   ```

   **コミット SHA の書き方 (リンク化の必須条件)**:
   - SHA の前後は半角スペースまたは行頭・行末で区切る。括弧・句読点・記号に密着させると GitHub がコミットリンクに変換しない
   - NG: `（9989016fb）` / OK: `（ 9989016fb ）`
   - 推奨は上記フォーマットのとおり文頭に裸で書く形: `9989016fb で対応しました！`
